...
import { actOnFlag, createFlag, dismissFlag, listOpenFlags } from './moderation';
...
describe('moderation', () => {
  ...
  it("actOnFlag('hide') hides the listing and reviews the flag", async () => {
    ...
    const listing = (await db.select().from(schema.listing).where(eq(schema.listing.id, listingId)).limit(1))[0];
    expect(listing.status).toBe('hidden');
  });

  it("actOnFlag('mature') sets mature=true on the listing", async () => {
    // Re-publish for this test
    await db.update(schema.listing).set({ status: 'published', mature: false }).where(eq(schema.listing.id, listingId));
    const { id } = await createFlag(db, { listingId, reporterId: null, kind: 'mature' });
