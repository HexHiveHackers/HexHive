  ...
  async function submit(e: SubmitEvent) {
    ...
    try {
      const input = {
        ...
        prerequisites: splitList(form.prerequisitesText),
        targetedVersions: form.targetedVersions,
      ...
      const finalizeRes = await fetch('/api/uploads/finalize', {
        method: 'POST', headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          type: 'script', listingId, versionId,
...
<section class="mx-auto max-w-2xl px-4 py-10">
  <h1 class="font-display text-2xl mb-6">Upload Scripts</h1>
  <form onsubmit={submit} class="grid gap-6">
    <ScriptForm bind:value={form} />
    <div>
      <label class="text-sm font-medium block mb-2">Script files</label>
      <FileDropzone bind:files accept=".s,.txt,.md,.py,.c,.h,.json,.zip" />
    </div>
